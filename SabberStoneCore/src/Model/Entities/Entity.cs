		...
		public virtual int this[GameTag t]
		{
			...
			set
			{
				...
				if (_history && (int)t < 1000)
					//if (Card.Type == CardType.HERO && t == GameTag.ZONE && value == (int)Enums.Zone.SETASIDE)
					//	//Game.PowerHistory.Add(PowerHistoryBuilder.FullEntity((IPlayable)this));
					...
					//				[GameTag.CARDTYPE] = (int)Card.Type
					//			},
		...
		public static IPlayable FromCard(Controller controller, Card card, IDictionary<GameTag, int> tags = null, IZone zone = null, int id = -1, int zonePos = -1)
		{
			try
			{
				...
				catch (Exception e)
				{
					...
					Console.WriteLine($"There was an error adding this result to the IdEntityDic. result = {result} result.Id = {result.Id}");
					throw e;
				}
					...
					if (zone is DeckZone)
					{
						var toAdd = new PowerHistoryFullEntity
						{
							Entity = new PowerHistoryEntity
							{
								...
								card = result.Card,
								Player_id = result.Controller.PlayerId,
							}
						};

						if (result.IsPlayable)
							toAdd.Entity.zone = result.Zone.Type;
			...
			{
				throw new Exception($"Tags {tags} probably caused an issue!");
			}
		}
